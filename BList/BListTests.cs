...
namespace Testing
{
    ...
    public class BListTests
    {
        ...
        [Test]
        public void given_existing_blist_when_elements_are_inserted_together_at_index_zero_then_blist_iterator_should_return_correct_latest_elements()
        {
            var initial = new int[512].Select(_ => _random.Next()).ToArray();
            var insertzero = new int[256].Select(_ => _random.Next()).ToArray();
            var expected = insertzero.Concat(initial).ToArray();
        ...
        [Test]
        public void given_existing_blist_when_multiple_elements_inserted_at_end_then_blist_iteractor_should_return_elements()
        {
            var initial = new int[256].Select(_ => _random.Next()).ToArray();
            var expected = initial.Concat(initial).ToArray();
            ...
            blist.AddRange(initial);
            blist.Insert(blist.Count, initial);
